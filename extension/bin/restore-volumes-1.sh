...
sudo dnf install --assumeyes rsync dvdisaster genisoimage &&
    ...
    do
        aws s3 cp s3://hp-pavillion/volumes.tar.gz.${I}.gpg.iso hp-pavillion/volumes.tar.gz.${I}.gpg.iso &&
            docker container run --interactive --rm --volume $(pwd):/in:ro --volume /srv/host/home/user:/out alpine:3.4 cp /in/hp-pavillion/volumes.tar.gz.${I}.gpg.iso /out/volumes.tar.gz.${I}.gpg.iso &&
